...
let kingInCheckColor = null;    // "white", "black", or null

function isKingInCheck(color) {
  // find the king’s square
  const kingImg   = $(`img[data-piece="${color}king"]`)[0];
  const kingCell  = $(kingImg).closest('.gamecell');
  const [kx, ky]  = kingCell.attr('id').split('_').map(Number);

  // for every opposing piece on the board, see if it could move to kx,ky
  const enemyColor = color === 'white' ? 'black' : 'white';
  let inCheck = false;
  $(`img[data-color="${enemyColor}"]`).each(function() {
    const fromCell = $(this).closest('.gamecell');
    const [fx, fy] = fromCell.attr('id').split('_').map(Number);
    const pieceType = $(this).data('piece'); // e.g. "queen","rook",etc.

    // reuse your existing move‑validation function
    if (canMove(pieceType, fx, fy, kx, ky, enemyColor)) {
      inCheck = true;
